...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 LUCENT
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 LUCENT over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 LUCENT+1 ( bit on if present )
            // bit 1 - 1000 LUCENT+1
            // bit 2 - 100 LUCENT+1
            // bit 3 - 10 LUCENT+1
            // bit 4 - 1 LUCENT+1
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 3000 LUCENT.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 LUCENT.");
                    } else {
...
void CWallet::AutoZeromint()
{
    ...
    // zLUCENT are integers > 0, so we can't mint 10% of 9 LUCENT
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zLUCENT\n", nBalance);
        return;
    }

    // Percentage of zLUCENT we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zLUCENT (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zLUCENT amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zLUCENT amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zLUCENT We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough LUCENT to mint this denomination
    ...
    CWalletTx wtx;
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zLUCENT. Current percentage of zLUCENT: %lf%%\n",
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CZerocoinMint>& vMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to LUCENT
    ...
    // Sign if these are lucent outputs - NOTE that zLUCENT outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256& hashTxOut, CTxIn& newTxIn, CZerocoinSpendReceipt& receipt)
{
    ...
    receipt.SetStatus("Transaction Mint Started", ZLUCENT_TXMINT_GENERAL);
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus("the selected mint coin is an invalid coin", ZLUCENT_INVALID_COIN);
        return false;
    }
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason)) {
        receipt.SetStatus("Try to spend with a higher security level to include more coins", ZLUCENT_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus("the new spend coin transaction did not verify", ZLUCENT_INVALID_WITNESS);
            return false;
        }
        ...
        //Add the coin spend into a Lucent transaction
        ...
        try {
            ...
            receipt.SetStatus("failed to deserialize", ZLUCENT_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (!newSpendChecking.Verify(accumulator)) {
            receipt.SetStatus("the transaction did not verify", ZLUCENT_BAD_SERIALIZATION);
            return false;
        }
        ...
        for (const CBigNum& item : listCoinSpendSerial) {
            if (spend.getCoinSerialNumber() == item) {
                //Tried to spend an already spent zLUCENT
                ...
                receipt.SetStatus("the coin spend has been used", ZLUCENT_SPENT_USED_ZLUCENT);
                return false;
            }
        }
        ...
        receipt.SetStatus("CoinSpend: Accumulator witness does not verify", ZLUCENT_INVALID_WITNESS);
        return false;
    }

    receipt.SetStatus("Spend Valid", ZLUCENT_SPEND_OKAY); // Everything okay
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CZerocoinMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    int nStatus = ZLUCENT_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus("Value is below the the smallest available denomination (= 1) of zLUCENT", nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZLUCENT_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zLUCENT transaction
    if (vSelectedMints.empty()) {
        ...
        // Select the zLUCENT mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to lucent address to the transaction (the actual primary spend taking place)
    ...
    receipt.SetStatus("Transaction Created", ZLUCENT_SPEND_OKAY); // Everything okay
...
void CWallet::ZLUCENTBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozLUCENTbackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozLUCENTbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozLUCENTbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    int nStatus = ZLUCENT_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZLUCENT_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZLUCENT_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& mint : vNewMints) {
            if (!walletdb.EraseZerocoinMint(mint)) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZLUCENT_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    receipt.SetStatus("Spend Successful", ZLUCENT_SPEND_OKAY);  // When we reach this point spending zLUCENT was successful
