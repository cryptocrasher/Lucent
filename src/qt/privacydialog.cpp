...
#include "zLUCENTcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zLUCENT ought to be enough for anybody." - Bill Gates, 2017
    ui->zLUCENTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzLUCENTSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzLUCENT->setEnabled(false);
        ui->pushButtonMintzLUCENT->setToolTip(tr("zLUCENT is currently disabled due to maintenance."));

        ui->pushButtonSpendzLUCENT->setEnabled(false);
        ui->pushButtonSpendzLUCENT->setToolTip(tr("zLUCENT is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zLUCENTpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzLUCENT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLUCENT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zLUCENT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zLUCENT in ") + 
...
void PrivacyDialog::on_pushButtonSpendzLUCENT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLUCENT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zLUCENT
        sendzLUCENT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zLUCENT
    sendzLUCENT();
}

void PrivacyDialog::on_pushButtonZLUCENTControl_clicked()
{
    ZLUCENTControlDialog* zLUCENTControl = new ZLUCENTControlDialog(this);
    zLUCENTControl->setModel(walletModel);
    zLUCENTControl->exec();
}

void PrivacyDialog::setZLUCENTControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzLUCENTSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzLUCENT()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Lucent Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zLUCENTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zLUCENT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " LUCENT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zLUCENT</b>";
    ...
    // use mints from zLUCENT selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZLUCENTControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZLUCENTControlDialog::GetSelectedMints();
    }
    ...
    // Clear zLUCENT selector in case it was used
    ZLUCENTControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zLUCENT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " LUCENT, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zLUCENT Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zLUCENT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zLUCENT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zLUCENT "));
    ui->labelzLUCENTAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzLUCENTSyncStatus->setVisible(fShow);
}
