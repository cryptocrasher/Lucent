// Copyright (c) 2017 The Lucent developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zLUCENTcontroldialog.h"
#include "ui_zLUCENTcontroldialog.h"
...
std::list<std::string> ZLUCENTControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZLUCENTControlDialog::listMints;

ZLUCENTControlDialog::ZLUCENTControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZLUCENTControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZLUCENTControlDialog::~ZLUCENTControlDialog()
{
    delete ui;
}

void ZLUCENTControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZLUCENTControlDialog::updateList()
{
...
void ZLUCENTControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZLUCENTControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZLUCENT_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZLUCENTControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZLUCENTControlDialog::GetSelectedMints()
{
...
void ZLUCENTControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
